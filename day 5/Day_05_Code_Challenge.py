...
if re.search(r'[^\+^\-]?[0123456789]*\.{1}[0123456789]+$',input1):
    print('True')    
else:
    print("False")
...
print(list1)
for each in list1:
    if re.search(r'^[a-zA-Z]{1}[a-zA-Z0-9\.\-\_]{0,}@[a-zA-Z0-9]{1,}\.[a-zA-Z]{2,4}$',each):
        pass
    else:
        list1.remove(each)
print(sorted(list1))
...
if re.search(r'^[456]{1}[0-9]{3}\-?[0-9]{4}\-?[0-9]{4}\-?[0-9]{4}$',input1):
    for each in input1:
        if each is '-':
            pass
        else:
    ...
    for x in range(0,14):
        ...
        c=x+3
        d=x+4
        if(list1[x]==list1[a] and list1[a]==list1[b] and list1[b]==list1[c]):
            flag=1
            exit  
        else:
            pass
...
"""
import re
with open("simpsons_phone_book.txt") as f1:
    for items in f1:
        if(re.search(r'^(J){1}[a-zA-Z]* (Neu)',items)):
            print(items)
