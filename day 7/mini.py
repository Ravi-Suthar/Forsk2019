...
First send the data to cloud using the "http://13.127.155.43/api_v0.1/sending" api 
with the following fields (Field names should be same as shown ):
...
The first one will give response-code : 
    200 and response-message : "Data added Successfully", 
    if all goes well. The second one will give all the details stored 
    with the phone number you provided as query parameter. Both the responses will be in JSON format.:
        """
        
import json
import requests
Host = "https://envnk8gfdse9.x.pipedream.net/"

data = {"Phone_Number":"7425967157","Name":"Ravi","College_Name":"PGI","Branch":"CSE"}
...
import json
import requests
response = requests.get("https://en8jnm5ziyd28.x.pipedream.net//?'Phone_Number'='7425967157'")
